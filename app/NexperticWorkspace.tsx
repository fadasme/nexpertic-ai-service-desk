...
function KnowledgeView({ articles }: { articles: KnowledgeArticle[] }) {
  return <><PageTitle title="Base de conocimiento" text="Artículos validados para soporte y respuesta asistida." action={<button className="nxPrimaryAction" type="button">+ Nuevo artículo</button>}/><div className="nxToolbar"><button type="button">Categoría</button><button type="button">Sección</button><button type="button">Estado</button><button type="button">Palabras clave</button></div><div className="nxPanel nxDataPanel"><div className="nxTableWrap"><table><thead><tr><th>Artículo</th><th>Estado</th><th>Categoría</th><th>Calidad</th><th>Usos</th></tr></thead><tbody>{articles.map((article) => <tr key={article.id}><td><b>{article.title}</b><small>{article.summary}</small></td><td><span className={article.status === "Validado" ? "nxOnline" : "nxPending"}>{article.status}</span></td><td>{article.domain}</td><td>{article.qualityScore}%</td><td>{article.uses}</td></tr>)}</tbody></table></div>{articles.length === 0 && <EmptyState icon="book" title="No hay artículos" text="Crea el primer artículo para alimentar la asistencia IA."/>}</div></>;
}
